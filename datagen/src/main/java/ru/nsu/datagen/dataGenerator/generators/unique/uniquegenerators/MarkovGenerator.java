package ru.nsu.datagen.dataGenerator.generators.unique.Generators;
...
public class MarkovGenerator implements UniqueKeyGenerator {
    ...
    private static List<Map<String, Double>> padColumnsToEqualLength(
                List<Map<String, Double>> columns,
                String nullPrefix
        ) {
            if (columns.isEmpty()) return columns;
            
            int targetLen = columns.stream().mapToInt(Map::size).max().orElse(0);

            
            List<Map<String, Double>> padded = new ArrayList<>();
            
            for (int colIdx = 0; colIdx < columns.size(); colIdx++) {
                Map<String, Double> col = new HashMap<>(columns.get(colIdx));
                ...
                while (deficit > 0) {
                    ...
                    if (!col.containsKey(candidate)) {
                        ...
                        if (nullWeight != null) {
                            assigned = nullWeight;
                        } else if ("min".equals(nullWeightPolicy)) {
                            assigned = col.values().stream()
                                .filter(v -> v > 0)
                                .min(Double::compare)
                                .orElse(1.0);
                        } else { // epsilon
